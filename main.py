...
# ── Import our modules ────────────────────────────────────────────────────────
from rag import get_stats as rag_stats
from rag import retrieve as rag_retrieve
from translator import (SUPPORTED_LANGUAGES, detect_and_translate_input,
                        language_display, translate_output_to_english,
...
def summarize_query(
    ...
    if is_ollama_model(deployment):
        print("🦙 Using Ollama locally...")
        raw = call_ollama(
            model=deployment,
        ...
        print("☁️ Using OpenAI / Cloud model...")
        response = client.chat.completions.create(
            model=deployment,
            messages=[
...
def is_ollama_model(model_name: str) -> bool:
    return ":" in model_name  # e.g. phi:latest

def call_ollama(model: str, system_prompt: str, user_content):
    payload = {
        "model": model,
